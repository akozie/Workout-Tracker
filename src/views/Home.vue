<template>
  ...
  <div v-if="data.length === 0" class="w-full flex flex-col items-center">
    ...
    <router-link :to="{name: 'Create'}"  class="mt-6 py-2 px-6 rounded-sm text-s m
      ...
      <!-- Data -->
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-col-4 gap-6">
        ...
        <p class="mt-6 py-1 px-3 text-xs bg-at-light-green shadow-md rounded-lg"> {{workout.workoutType}} </p>
        <h1 class="mt-8 mb-2 text-center text-xl bg-at-light"> {{workout.workoutName}} </h1>
        </router-link>  
      </div>
  </div>
</template>
