          ...
          <div class="mt-10 p-8 rounded-md flex flex-col item-center bg-light-grey shadow-md">
            ...
            <div v-if="data.workoutType === 'strength'" class="flex flex-col gap-y-4 w-full">
                <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row" v-for="(item, index) in data.exercises" :key="index">
                  ...
                  <input id="sets" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.sets">
                  <p v-else>{{item.sets}}</p>
                  </div>
                  <div class="flex flex-1 flex-col">
                  <label for="reps" class="mb-1 text-sm text-at-light-green"> Reps </label>
                  <input id="reps" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.reps">
                  <p v-else>{{item.reps}}</p>
                  </div>
                  <div class="flex flex-1 flex-col">
                  <label for="weight" class="mb-1 text-sm text-at-light-green"> Weight </label>
                  <input id="weight" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.weight">
        ...
        Add Exercise
            ...
            <div v-else class="flex flex-col gap-y-4 w-full">
                ...
                <option value="walk">Walk</option>
                  ...
                  <input id="distance" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.distance">
                  <p v-else>{{item.distance}}</p>
                  </div>
                  <div class="flex flex-1 flex-col">
                  <label for="duration" class="mb-1 text-sm text-at-light-green"> Duration </label>
                  <input id="duration" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.duration">
                  <p v-else>{{item.duration}}</p>
                  </div>
                  <div class="flex flex-1 flex-col">
                  <label for="pace" class="mb-1 text-sm text-at-light-green"> Pace </label>
                  <input id="pace" v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.pace">
                  <p v-else>{{item.pace}}</p>
                  </div>
                  <img 
                 @click="deleteExercise(item.id)" v-if="edit" class="absolute h-4 w-auto -left-5 cursor-pointer" src="../assets/images/trash-light-green.png" alt="">
                </div>
                 <button
                 @click="addExercise"
                 v-if="edit"
...
export default {
  ...
  setup() {
    ...
    return {statusMsg, data, errorMsg, dataLoaded, edit, editMode, user, deleteWorkout, addExercise, deleteExercise, update};
  },
};
</script>
